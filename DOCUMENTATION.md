...
├── app.env                 # Runtime configuration (generated at setup)
│
...
| **Virtual Environment** | Creates `.env/` Python venv if not present |
| **Dependencies** | Runs `pip install -r requirements.txt` inside the venv |
| **Configuration** | Copies `app.env.example` → `app.env`, generates a random `APP_SECRET_KEY` |
...
### Step 4 — Configuration (`app.env`)

All runtime settings live in `app.env` in the project root. Edit this file to customise your installation:
...
DB_PATH=data/analytics_workbench.db
...
After editing `app.env`, restart the application:
...
**Fix:**
1. Ensure `app.env` exists in the project root. If missing, copy the example file:
   ```bash
...
| Password was reset and new one is unknown | Run `./setup.sh --reset-admin` to recreate the default `admin` / `admin` superadmin |
| `APP_SECRET_KEY` changed after initial setup | The bcrypt hashes are independent of `APP_SECRET_KEY`; this only affects API keys — check `app.env` for typos |
...
API keys are encrypted using a key derived from `APP_SECRET_KEY`. If you move your installation to a new server you **must** copy `app.env` alongside the database file — both must match.

Checklist when migrating:

- [ ] Copy `data/analytics_workbench.db` to the new server
- [ ] Copy `app.env` (with the same `APP_SECRET_KEY`) to the new server
