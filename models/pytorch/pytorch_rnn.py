...
class _RNNNetwork(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, hidden_units: int = 32):
        super().__init__()
        self.rnn = nn.RNN(input_dim, hidden_units, batch_first=True)
...
class PytorchRNNModel(BaseModel):

    def __init__(self):
        super().__init__(name="PyTorch RNN", framework="pytorch")
        self._build_config = {}
    ...
    def build(self, input_dim: int = None, output_dim: int = None, **kwargs) -> None:
        ...
        self.model = _RNNNetwork(input_dim, output_dim, hidden_units)
